    ...
    def __init__(self, params):
        ...
        if (params.Integrator.type == "Verlet"):
            if(params.Langevin.on):
                ...
                if(params.Integrator.order >= 10):
                    self.update = Symplectic
                if(params.Integrator.order == 0):
                    self.update = Verlet
...
def Symplectic(ptcls, params):
    dt = params.Control.dt
    order = params.Integrator.order
    c = params.Integrator.c
    d = params.Integrator.d

    n = len(c)

    if(order == 10):
        ptcls.vel = ptcls.vel + dt*c[0]*ptcls.acc
        ptcls.pos = ptcls.pos + dt*d[0]*ptcls.vel
        if params.Control.PBC == 1:
            enforce_pbc(ptcls.pos, params.Lv) 
        ...
        for i in range(0, n-1):
            ...
            ptcls.vel = ptcls.vel + dt*c[i]*ptcls.acc
            ptcls.pos = ptcls.pos + dt*d[i]*ptcls.vel

            if params.Control.PBC == 1:
                enforce_pbc(ptcls.pos, params.Lv) 

            U = calc_pot_acc(ptcls, params)
        ptcls.vel = ptcls.vel + dt*c[-1]*ptcls.acc
