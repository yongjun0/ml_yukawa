...
class Verbose:
    ...
    def sim_setting_summary(self):
        ...
        print('units: ', params.units, file = f_log)
        print('Temperature = {:2.6e} [K]'.format(params.T_desired) , file=f_log)
        
        print('\nNo. of non-zero box dimensions = ', int(params.d) , file=f_log)
        print('aws: Wigner-Seitz radius = {:2.6e}'.format(params.aws) , file=f_log)
        print('Box length along x axis = {:2.6e} = {:2.6e} a_ws'.format(params.Lv[0],params.Lv[0]/params.aws) , file=f_log)
        print('Box length along y axis = {:2.6e} = {:2.6e} a_ws'.format(params.Lv[1],params.Lv[1]/params.aws) , file=f_log)
        print('Box length along z axis = {:2.6e} = {:2.6e} a_ws'.format(params.Lv[2],params.Lv[2]/params.aws) , file=f_log)
        ...
        if (params.Magnetic.on):
            ...
            for ic in range(params.num_species):
                print('Cyclotron frequency of Species {:2} = {:2.6e}'.format( ic + 1, params.species[ic].omega_c) , file=f_log)
                print('beta_c of Species {:2} = {:2.6e}'.format( ic + 1, params.species[ic].omega_c/params.species[ic].wp) , file=f_log)

        print("\nAlgorithm = ", params.Potential.method , file=f_log)
        if (params.Potential.method == 'P3M'):
            print('Ewald_parameter = {:2.6e}'.format(params.Potential.matrix[-1,0,0]) , file=f_log)
            ...
            print('rcut = {:2.6e}'.format(params.Potential.rc) , file=f_log)
        ...
        print('\nSmallest interval in Fourier space for S(q,w): qa_min = {:2.6e}'.format( 2.0*np.pi*params.aws/params.Lx) , file=f_log)
    ...
    def time_stamp(self, time_stamp):
        ...
        if (self.params.Magnetic.on and self.params.Magnetic.elec_therm):
            ...
            print('Time for magnetic equilibration = {} hrs {} mins {} secs'.format( mag_eq_hrs, mag_eq_min, mag_eq_sec ) , file=f_log)
